...
function enter(pi) {
    ...
    if (pCol == parseInt(comb.substring(pRow, pRow + 1), 10)) {    //climb
        ...
        pRow--;
        pi.playPortalSound();
        // parseInt, because pRow here is whatever the row was minus one and the division is
        // rarely exact. warp only takes (int, int) or (int, String), and the engine will not
        // narrow a fractional double to either -- the call throws and nobody gets moved.
        // The climb branch above is safe: it only divides when pRow % 4 == 0.
